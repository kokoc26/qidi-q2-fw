...
class Heater:
    def __init__(self, config, sensor):
        ...
        self.z_max_limit = config.getfloat('z_max_limit', 999.)
        ...
        gcode.register_mux_command("CLOSE_CHAMBER_DEBUG","HEATER",
                                    short_name,self.cmd_close_chamber_debug)
        ####copy
        self.can_wait = True

    def cmd_close_chamber_debug(self, gcmd):
        heater_chamber = self.printer.lookup_object("heater_generic chamber")
        is_close = gcmd.get_int('S', 1)
        heater_chamber.heat_with_heater_bed = is_close
    ...
    def temperature_callback(self, read_time, temp):
        ...
        if position_z > self.z_max_limit: #and "xyz" in toolhead.get_status(curtime)["homed_axes"]:
            if heater.target_temp > 0.:
                heaters.set_temperature(heater, 0.)

#        logging.debug("temp11: %.3f %f = %f", position_z, temp)
    ...
    def cmd_SET_HEATER_TEMPERATURE(self, gcmd):
        ...
        if self.name == "heater_generic chamber" :
            ...
            if self.heat_with_heater_bed and temp > 0.:
                ...
                if heater_bed.target_temp < (temp+self.heat_with_heater_bed_tem_add):
                    ...
                    logging.debug("temp11: %.3f %f = %f", temp, temp)
...
class ControlPID:
    ...
    def temperature_update(self, read_time, temp, target_temp):
        ...
        if self.heater.name == "heater_generic chamber" and heater_bed.heater_bed_state != 2 and self.heater.heat_with_heater_bed == 1:
            self.heater.set_pwm(read_time, 0.)
        else:
...
class PrinterHeaters:
    def __init__(self, config):
        ...
        gcode.register_command("ABORT_TEMPERATURE_WAITS", self.cmd_ABORT_TEMPERATURE_WAITS,
                               desc="Abort any active temperature waits")
    ...
    def load_config(self, config):
        ...
        try:
            logging.info("MKS_AIR_DEBUG:enter 4")
            dconfig = pconfig.read_config(filename)
            logging.info("MKS_AIR_DEBUG:dconfig=%s, filename=%s" % (dconfig, filename))
        except Exception:
            logging.info("MKS_AIR_DEBUG:enter 5")
...
#        logging.info("EEEEEEE %s", heater_name)
    ...
    def abort_waits(self):
        # 递增世代计数，只有当前正在等待的循环会被打断
        self.abort_epoch += 1
    def cmd_ABORT_TEMPERATURE_WAITS(self, gcmd):
        self.abort_waits()
    ...
    def _wait_for_temperature(self, heater):
        ...
        local_epoch = self.abort_epoch
        while not self.printer.is_shutdown() and heater.check_busy(eventtime):
            if self.abort_epoch != local_epoch:
                return
