...
class ProbeCommandHelper:
    def __init__(self, config, probe, query_endstop=None):
        ...
        gcode.register_command('TESTING_PEI',
                               self.cmd_TESTING_PEI,
                               desc=self.cmd_TESTING_PEI_help)
    ...
    cmd_TESTING_PEI_help = "Testing the PEI"
    def cmd_TESTING_PEI(self, gcmd):
        # 判断是否开启
        if not self.printer.lookup_object('save_variables').allVariables.get('enable_usually_pei_detected'):
            gcmd.respond_info(f"没开启pei检测")
            return
        # 1. get the tartget pos
        X = gcmd.get_float('X', 0)
        Y = gcmd.get_float('Y', 0)
        Z = gcmd.get_float('Z', 0)
        # 2. level the motion
        self._move([X, Y, 5], 100.)
        # 3. vertical the motion
        pos = self.printer.lookup_object('toolhead').get_position()
        pos[2] = Z
        gcmd.respond_info("pos: %s" % (pos,))
        try:
            epos = self.probe.mcu_probe.probing_move(pos, 1.)
        except self.printer.command_error as e:
            # 3.1 if error return false 
            self._move([X, Y, 5], 100.)
            raise gcmd.error("pei detecting error")

        # 3.2 if no error return true
        self._move([X, Y, 5], 100.)
...
class ProbeSessionHelper:
    ...
    def run_probe(self, gcmd):
        ...
        while len(positions) < sample_count:
            # Probe position
            pos = self._probe(params['probe_speed'])  # 执行探点的动作
            positions.append(pos)
            
            # gcmd.respond_info("probe at %.3f,%.3f is z=%.6f" % (pos[0], pos[1], pos[2]))
            ...
            # if pos[2] > 1:
            #     if pos[2] < 4:
            #         gcmd.respond_info("Test retry:%.3f" % pos[2])
            #         retries += 1
            #         self.mcu_probe.home_zero()
            #         positions = []
        ...
        while 1:
            ...
            while retry_count <= max_retries and not success:
                try:
                    ...
                    reason = str(e)
                    if "No trigger on" in reason:
                        if retry_count >= max_retries:
                            gcmd.respond_raw("!! Homing failed after %d retries: %s" % (max_retries, reason))
                            raise  # 重试次数用完，抛出原始异常
                        else:
                            # 记录重试信息
                            retry_count += 1
                            retry_msg = "Retrying homing (%d/%d) after error: %s" % (retry_count, max_retries, reason)
                            gcmd.respond_raw(retry_msg)
                    else:
                        raise
            # -------------------z探针重试逻辑 --------------------------
            probe_num += 1
