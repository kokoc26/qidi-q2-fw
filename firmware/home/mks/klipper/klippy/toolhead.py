...
class ToolHead:
    def __init__(self, config):
        ...
        self.lookahead.set_flush_time(LOOKAHEAD_FLUSH_TIME)
    ...
    def _estimated_print_time(self, curtime):
        pt_max = max(mcu.estimated_print_time(curtime) for mcu in self.all_mcus)
        return pt_max
    ...
    def _calc_print_time(self):
        ...
        est_print_time = self._estimated_print_time(curtime)
    ...
    def _check_pause(self):
        ...
        est_print_time = self._estimated_print_time(eventtime)
    ...
    def wait_moves(self):
        ...
        while (not self.special_queuing_state
               or self.print_time >= self._estimated_print_time(eventtime)):
            if not self.can_pause:
                break
    ...
    def get_status(self, eventtime):
        print_time = self.print_time
        estimated_print_time = self._estimated_print_time(eventtime)
    ...
    def cmd_SET_VELOCITY_LIMIT(self, gcmd):
        ...
        self.printer.set_rollover_info("toolhead", "toolhead: %s" % (msg,),False) #屏蔽输出
