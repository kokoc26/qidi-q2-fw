    ...
    try:
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            ...
            if os.path.exists(tmp_project_settings_path):
                ...
                metadata['preset_colours'] = project_settings_json.get('filament_colour', '') or ''
                metadata['layer_height'] = project_settings_json.get('layer_height', '') or ''
                metadata['nozzle_temperature_initial_layer'] = project_settings_json.get('first_layer_extr_temp', '') or ''

                # 如果这些字段是列表类型，需要连接成字符串，缺失时设为空字符串
                metadata['filament_type'] = ";".join(project_settings_json.get('filament_type', [])) if project_settings_json.get('filament_type') else ''
                metadata['filament_name'] = ";".join(project_settings_json.get('filament_settings_id', [])) if project_settings_json.get('filament_settings_id') else ''
