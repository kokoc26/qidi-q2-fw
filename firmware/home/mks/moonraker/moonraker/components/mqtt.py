...
SERVERS_NAME = ["aliyun", "aws"]
SERVERS = {
    "aliyun":{
        "address": "47.120.29.134",
        "port": 1883,
        "client_id": "Q2@192.168.21.158",
        "enable_tls": False,
        "username": None,
        "password": None,
        "password_file": None,
    ...
    "aws":{
        "address": "47.120.29.134",
        "port": 1883,
        "client_id": "Q2_aws@192.168.21.158",
        "enable_tls": False,
        "username": None,
        "password": None,
        "password_file": None,
...
class MQTTClient(APITransport):
    def __init__(self, config: ConfigHelper) -> None:
        self.automatic_reconnection_flag = True
        self.server = config.get_server()
        self.eventloop = self.server.get_event_loop()



        self.switch_server_information("aliyun")
        ...
        self.server.register_endpoint(
            "/server/mqtt/connect", RequestType.GET,
            self._handle_connect_request,
            transports=ep_transports
        )
        self.server.register_endpoint(
            "/server/mqtt/disconnect", RequestType.GET,
            self._handle_disconnect_request,
            transports=ep_transports
        )
        self.server.register_endpoint(
            "/server/mqtt/server_change", RequestType.POST,
            self._handle_change_server_request,
            transports=ep_transports
        )
        ...
        for key, val in self.status_cfg.items():
            if val is not None:
    ...
    def switch_server_information(self, server_name: str):
        server_config = SERVERS.get(server_name)
        self.address: str = server_config.get("address", "47.120.29.134")
        self.port: int = server_config.get("port", 1883)
        self.tls_enabled: bool = server_config.get("enable_tls", False)
        self.user = server_config.get("username", None)
        self.pw_file_path = server_config.get("password_file", None)
        self.pw_template = server_config.get("password", None)
        self.protocol = server_config.get("mqtt_protocol", "v3.1.1")
        self.instance_name = server_config.get("instance_name", socket.gethostname())
        self.qos = server_config.get("default_qos", 0)
        self.publish_split_status = server_config.get("publish_split_status", False)
        self.client_id: Optional[str] = server_config.get("client_id", None)
        self.status_cfg: Dict[str, str] = server_config.get("status_objects", {})
        self.status_interval = server_config.get("status_interval", 0)
        self.api_qos = server_config.get('api_qos', self.qos)


        self.user_name: Optional[str] = None
        if self.user:
            self.user_name = self.user.render()
        self.password: Optional[str] = None
        if self.pw_file_path is not None:
            pw_file = pathlib.Path(self.pw_file_path).expanduser().absolute()

            self.password = pw_file.read_text().strip()
        if self.pw_template is not None:
            self.password = self.pw_template.render()
        self.protocol = MQTT_PROTOCOLS.get(self.protocol, None)
        if PAHO_MQTT_VERSION < (2, 0):
            self.client = ExtPahoClient(self.client_id, protocol=self.protocol)
        else:
            self.client = ExtPahoClient(
                paho_mqtt.CallbackAPIVersion.VERSION1, self.client_id,
        ...
        if server_config.get("enable_moonraker_api", True):
    ...
    def _on_connect(self,
        ...
        if reason_code == 0:
            ...
            self.server.remove_warning("mqtt_disconnect_reson")
            self.server.remove_failed_component("mqtt")
            ...
            self.server.add_warning(f"MQTT Connection Failed: {err_str}", "mqtt_disconnect_reson")
    ...
    def _on_disconnect(self,
        ...
        if self.disconnect_evt is not None:
            ...
            if self.connect_task is None and self.automatic_reconnection_flag:
        ...
        while True:
            ...
            try:
                ...
                if type(last_err) is not type(e) or last_err.args != e.args:
                    ...
                    last_err = e
                continue
            temp = await self.wait_connection(2.0)
            if not temp:
                logging.info("MQTT connection timeout, retrying...")
                continue  # 超时则继续 loop，等待下一次重试
    ...
    async def _handle_connect_request(self, web_request: WebRequest):
        self.automatic_reconnection_flag = True
        ...
        success = await self.wait_connection(5.)
        result = {"result": "ok"} if success else {"result": "error"}
        return result


    async def _handle_disconnect_request(self, web_request: WebRequest):
        self.automatic_reconnection_flag = False
        if not self.is_connected():
            return {"result": "ok"}
        self.client.disconnect()
        return {"result": "ok"}

    async def _handle_change_server_request(self, web_request: WebRequest):
        name: str = web_request.get_str("server")
        if name is None :
            return {"result": "error"}
        ...
        if self.is_connected():
            self.client.disconnect()
        self.switch_server_information(name)
        await self.component_init()
