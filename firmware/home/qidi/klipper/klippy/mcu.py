...
class MCU:
    ...
    def __init__(self, config, clocksync):
        ...
        # Boot reason
        self.boot_reason = None    # TODO: add gcode command to get?
        # Register handlers
    ...
    def _connect(self):
        # get boot reason
        try:
            cmd = self.lookup_command("get_boot_reason")
            if cmd is not None:
                logging.debug("MCU %s supports boot reason query, requesting...", 
                            self._name)
                get_boot_reason = self.lookup_query_command(
                    "get_boot_reason",
                    "send_boot_reason flag=%u reg=%u")
                params = get_boot_reason.send()
                if params:
                    flag = int(params.get('flag', 0))
                    reg = int(params.get('reg', 0))
                    boot_info = self._parse_boot_reason(flag, reg)
                    self.boot_reason = boot_info
                    logging.info("MCU %s boot reason: %s", 
                            self._name, boot_info['full_text'])
        except Exception as e:
            if "Unknown command: get_boot_reason" in str(e):
                logging.debug("MCU %s does not support get boot reason...",
                            self._name)
            else:
                logging.error("Failed to get boot reason for MCU %s: %s", 
                            self._name, str(e))
    ...
    def _parse_boot_reason(self, flag, reg):
        reset_cause_map = {
            0: "UNKNOWN",
            1: "Low-power reset",
        ...
        reason = reset_cause_map.get(flag, f"UNKNOWN({flag})")
        reg_flags = []
        reg_int = int(reg)
        if reg_int & (1 << 31):  # bit 31: Low-power reset flag
            reg_flags.append("LOWPOWER")
        if reg_int & (1 << 30):  # bit 30: Window watchdog reset flag
            reg_flags.append("WWDG")
        if reg_int & (1 << 29):  # bit 29: Independent watchdog reset flag
            reg_flags.append("IWDG")
        if reg_int & (1 << 28):  # bit 28: Software reset flag
            reg_flags.append("SOFTWARE")
        if reg_int & (1 << 27):  # bit 27: POR/PDR reset flag
            reg_flags.append("POR_PDR")
        if reg_int & (1 << 26):  # bit 26: PIN reset flag
            reg_flags.append("PIN")
        if reg_int & (1 << 25):  # bit 25: BOR reset flag
            reg_flags.append("BOR")
        reg_hex = f"0x{reg_int:08X}"
        return {
            'flag': flag,
            'reason': reason,
            'reg_int': reg_int,
            'reg_hex': reg_hex,
            'reg_flags': reg_flags,
            'full_text': f"{reason} (flag={flag}, reg={reg_hex}, flags={','.join(reg_flags)})"
        }
