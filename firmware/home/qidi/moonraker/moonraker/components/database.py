...
from ..common import RequestType, SqlTableDefinition, JobEvent, KlippyState
import copy
...
SYSTEM_STATS_TABLE = "system_stats"

PRINT_STATE_TEMPLATE = {
    "extruder_target": "EXTRUDER_TEMP",
    "heater_bed_target": "BED_TEMP",
    "chamber_target": "CHAMBER_TEMP",
    "cooling_fan_speed": "FAN_SPEED",
    "auxiliary_cooling_fan_speed": "AUX_FAN_SPEED",
    "chamber_circulation_fan_speed": "CHAMBER_FAN_SPEED",
    "absolute_coord": "ABSOLUTE_COORD",
    "absolute_extrude": "ABSOLUTE_EXTRUDE",
    "base_position": "BASE_POSITION",
    "gcode_position": "LAST_POSITION",
    "homing_position": "HOME_POSITION",
    "speed": "SPEED",
...
def get_cpu_serial():
    ...
    try:
        ...
        logging.error(f"读取cpuinfo文件时出错: {e}")
        return '0000000000'

# 在定义配置字典前先拿到 SN
_sn = get_cpu_serial()
_alias_default = f"3DP-{_sn[:3]}-{_sn[-3:]}" 

DEFAULT_CONFIGA_ITEM = {
    "box.auto_read_rfid": "1",
    "box.auto_refill": "1",
    "box.self_test": "1",
    "ai.noodle_detection": "0",
    "ai.sensitivity": "HIGH",
    "ai.spool_detection": "0",
    "ai.foreign_object_detection": "0",
    "ai.bed_detection": "0",
    "printing.cache": "1",
    "printing.polar_cooler": "1",
    "general.video": "1",
    "general.sound": "1",
    "general.sleep_time": "5",
    "general.chamber_light": "1",
    "general.screen_state": "1",
    "general.indicator_light": "1",
    ...
    "user.token": "",
    "user.region": "NA",
    "user.alias": _alias_default,
    "mqtt.account": "",
    "mqtt.password": "",
    "nozzle.type": ["hardened_steel"],
    "nozzle.diameter": ["0.4"],
    "update.close_loop_hd_version": ""
}
...
class MoonrakerDatabase:
    def __init__(self, config: ConfigHelper) -> None:
        ...
        # config表默认值
        self.config_items = copy.deepcopy(DEFAULT_CONFIGA_ITEM)
        # systemStats的错误词典
        self._system_stats_error = {}  # 本地维护的错误词典/状态
        ...
        # TODO：访问config接口
        self.server.register_endpoint(
            "/server/database/config/select", RequestType.POST, self._handle_config_select_request
        )
        self.server.register_endpoint(
            "/server/database/config/select_all", RequestType.POST, self._handle_config_select_all_request
        )
        self.server.register_endpoint(
            "/server/database/config/update", RequestType.POST, self._handle_config_update_request
        )
        self.server.register_endpoint(
            "/server/database/config/delete", RequestType.DELETE, self._handle_config_delete_request
        )
        self.server.register_endpoint(
            "/server/database/config/update_batch", RequestType.POST, self._handle_config_update_batch_request
        )
        self.server.register_endpoint(
            "/server/database/system_stats/get_batch", RequestType.POST, self._handler_get_system_stats_by_range
        )
        # debug 接口
        ...
        # # 注册klipper信息接口
        # self.server.register_remote_method("get_value_from_config", self.test_connection_klipper)

        self.prune_system_stats_timer = self.eventloop.register_timer(self._check_and_prune)
        self.server.register_event_handler(
    ...
    async def _handle_started(self, state: KlippyState) -> None:
        ...
        sub: Dict[str, Optional[List[str]]] = {"closed_loop x": None, "closed_loop y": None}
        try:
            result = await kapis.subscribe_objects(sub, self._status_update)
        except self.server.error:
            logging.error("Error subscribing to closed_loop")
        await self._status_update(result, 0.0)


    async def _status_update(self, data: Dict[str, Any], _: float) -> None:
        stored_version = self.config_items.get("update.close_loop_hd_version")
        new_version = None
        for axis in ["closed_loop x", "closed_loop y"]:
            if axis in data and data[axis].get("stepper_hd_version"):
                new_version = data[axis].get("stepper_hd_version")
                break
        if not new_version:
            return
        if not stored_version or (new_version and new_version != stored_version):
            logging.info(f"Updating hardware version: {stored_version} -> {new_version}")
            self.config_items["update.close_loop_hd_version"] = new_version
            await self.update_value_from_config("update", "close_loop_hd_version", new_version)
        

    # 生成断电续打gcode
    async def generate_power_off_print_gcode(self) -> str:
        import json, base64
        # 1. 获取配置并初始化基础数据
        cur = await self.get_all_by_section_from_config("print_state")
        item = dict(await cur.fetchall() or {})
        
        # 2. 解析与限流逻辑处理
        try:
            all_objs = json.loads(item.get("all_objects", "[]"))
            exclude_objs = json.loads(item.get("exclude_objects", "[]"))
        except (json.JSONDecodeError, TypeError):
            all_objs = []
            exclude_objs = []

        # 如果对象总数超过 64，为了防止 G-code 指令过长导致 Klipper 缓冲区溢出或处理缓慢, 强制将对象列表和排除列表清空
        if len(all_objs) > 64:
            logging.warning(f"Object count ({len(all_objs)}) exceeds limit 64. Clearing object data for flow control.")
            all_objs = []
            exclude_objs = []
        else:
            # 只有在未超限时，才进行复杂的 polygon 抽稀处理
            for obj in all_objs:
                # 转换 Center 为 "x,y" 字符串
                if isinstance(obj.get('center'), (list, tuple)):
                    obj['center'] = ",".join(map(str, obj['center']))

                # 抽稀 Polygon (每5个点采样一次)
                poly = obj.get('polygon')
                if isinstance(poly, list) and len(poly) > 30:
                    simplified = poly[::5]
                    if poly[-1] != simplified[-1]:
                        simplified.append(poly[-1])
                    obj['polygon'] = json.dumps(simplified, separators=(',', ':'))

        # 更新 item 并打包
        item.update({
            "all_objects": all_objs,
            "exclude_objects": exclude_objs
        })

        for key in ["gcode_position", "homing_position", "base_position"]:
            try:
                raw = json.loads(item.get(key, '[0,0,0,0]'))
                item[key] = [(float(raw[i]) if i < len(raw) else 0.0) for i in range(4)]
            except Exception:
                logging.error(f"Failed to parse {key}")
                item[key] = [0.0] * 4

        state_data = {"params": item, "timestamp": time.time()}
        
        # 5. 序列化与 Base64 编码
        json_bytes = json.dumps(state_data, separators=(',', ':')).encode()
        encoded_state = base64.b64encode(json_bytes).decode()
        
        script = f"SDCARD_POWER_OFF_CONTINUES_PINT_FILE STATE={encoded_state}"
        logging.info(f"发送封包到 Klipper，长度: {len(script)}")
        return script
    ...
    async def component_init(self) -> None:
        ...
        await self._init_config_items()
        self.prune_system_stats_timer.start()
    ...
    async def _init_config_items(self):
        import json
        def _smart_decode(val):
            ...
            # 3. 递归清洗（处理列表/字典内部可能残留的 bytes）
            if isinstance(val, list):
                return [_smart_decode(i) for i in val]
            if isinstance(val, dict):
                return {k: _smart_decode(v) for k, v in val.items()}
            
            return val

        for key, value in self.config_items.items():
            parts = key.split('.')
            cur = await self.get_value_from_config(parts[0], parts[1])
            item = dict(await cur.fetchone() or {})
            if item:
                raw_val = item.get('value')
                self.config_items[key] = _smart_decode(raw_val)
                final_val = self.config_items[key]
                logging.info(f"Fixed item -> {key}: {final_val} (Type: {type(final_val)})")
    ...
    async def _on_job_state_changed(
        ...
        if job_event == JobEvent.STARTED:
            if history.current_job is not None:
                # 取消
                logging.info("任务取消")
                await self.delete_value_from_config("print_state", None)
        ...
        prev_state = prev_stats.get('state', 'unknown')
        new_state = new_stats.get('state', 'unknown')
        transition_str = f"{prev_state} -> {new_state}"
        self._system_stats_error["job_stats"] = transition_str
        self._system_stats_error["is_dirty"] = True
        logging.debug(f"System Monitor: State transition detected: {transition_str}")

    async def _sync_on_ready(self):
        try:
            # 1. 直接从已经同步好的本地字典获取
            led_mode = self.config_items.get("general.chamber_light")
            screen_state = self.config_items.get("general.screen_state")
            neopixel_enable = self.config_items.get("general.indicator_light")

            params = []
            
            # 2. 逻辑 A: led_mode 强制同步（没数据则为 1）
            if led_mode is not None:
                params.append(f"MODE={led_mode}")
            else:
                params.append("MODE=1")
            
            # 3. 逻辑 B: screen_state 只有在有数据时才传输
            if screen_state is not None:
                params.append(f"SCREEN={screen_state}")

            # 4. 发送指令
            kapis: KlippyAPI = self.server.lookup_component('klippy_apis')
            sync_cmd = f"SMART_STATUS {' '.join(params)}\nNEOPIXEL_ENABLE ENABLE={neopixel_enable}"
            
            await kapis.run_gcode(sync_cmd)
            logging.info(f"LED Sync from Local Cache: {sync_cmd}")
        except Exception as e:
            logging.error(f"Failed to sync LEDs from local cache: {str(e)}")
    
    def log_system_stats(
        self, 
        cpu_dict: Dict[str, float], 
        mem_dict: Dict[str, int]
    ) -> None:
        """
        高层业务接口：处理原始监控数据并持久化。
        由 proc_stats 每分钟调用一次。
        """
        try:
            # 1. 内部处理内存百分比
            total_mem = mem_dict.get("total", 0)
            used_mem = mem_dict.get("used", 0)
            mem_percent = 0.0
            if total_mem > 0:
            ...
            # 2. 判定并读取状态变迁信息（标志位逻辑）
            status_note = ""
            if self._system_stats_error.get("is_dirty", False):
                status_note = self._system_stats_error.get("job_stats", "")
                self._system_stats_error["is_dirty"] = False
            
            # 3. 封装最终存储字典
            payload = {
                "cpu_all": cpu_dict.get("cpu", 0.0),
                "cpu0": cpu_dict.get("cpu0", 0.0),
                "cpu1": cpu_dict.get("cpu1", 0.0),
                "cpu2": cpu_dict.get("cpu2", 0.0),
                "cpu3": cpu_dict.get("cpu3", 0.0),
                "memory": mem_percent,
                "error": status_note  # 记录瞬时的状态切换动作
            }

            # 4. 调用底层的异步插入接口
            self.insert_system_stats(payload)

        except Exception as e:
            logging.error(f"Database: Failed to log system stats: {e}")

    async def _check_and_prune(self, eventtime: float) -> float:
        """
        核心逻辑：检查是否跨天，若是则执行清理
        """
        current_date = time.strftime("%Y-%m-%d", time.localtime())
        last_cleanup = self._system_stats_error.get("date", "")
        if current_date != last_cleanup:
            try:
                count = await self.prune_system_stats(7)
                if count > 0:
                    logging.info(
                        f"System Monitor: Daily cleanup for {current_date} "
                        f"completed. Removed {count} old records."
                    )
                else:
                    logging.info(f"System Monitor: Cleanup checked for {current_date}, no records to remove.")
                self._system_stats_error["date"] = current_date
            except Exception:
                logging.exception("System Monitor: Cleanup failed")
        return eventtime + 3600.0
    ...
    # ***** Config table opertations *****
    def get_value_from_config(self, section: str, key: str)-> Future[Dict[str, Any]]:
        return self.db_provider.execute_db_function(
            self.db_provider.get_value_from_config, section, key
        )
    def update_value_from_config(self, section: str, key: str, value)-> Future[bool]:
        return self.db_provider.execute_db_function(
            self.db_provider.update_value_from_config, section, key, value
        )
    def update_value_from_config_batch(self, records: List[Tuple[str, str, Any]])-> Future[bool]:
        return self.db_provider.execute_db_function(
            self.db_provider.update_value_from_config_batch, records
        )
    def delete_value_from_config(self, section: str, key: str | None)-> Future[bool]:
        return self.db_provider.execute_db_function(
            self.db_provider.delete_value_from_config, section, key
        )
    def get_all_by_section_from_config(self, section: str)-> Future[Dict[str, Any]]:
        return self.db_provider.execute_db_function(
            self.db_provider.get_all_by_section_from_config, section
        )

    # ******** systemStats table opertations **********
    def insert_system_stats(
        ...
        return self.db_provider.execute_db_function(
            self.db_provider.insert_system_stats, 
            timestamp, 
            stats
        )
    def lookup_system_stats(
        self, start_time: int, end_time: int
    ) -> Future[List[Dict[str, Any]]]:
        """
        获取指定 Unix 时间戳区间内的统计记录。
        start_time / end_time 均为秒值。
        """
        return self.db_provider.execute_db_function(
            self.db_provider.lookup_system_stats,
            start_time,
            end_time
        )
    def prune_system_stats(self, days: int = 7) -> Future[int]:
        """
        删除超过指定天数的数据。返回被删除的行数。
        """
        return self.db_provider.execute_db_function(
            self.db_provider.prune_system_stats,
            days
        )
    ...
    # config表的查询接口
    async def _handle_config_select_request(self, web_request: WebRequest) -> Dict[str, Any]:
        try:
            section = web_request.get_str("section")
            key = web_request.get_str("key")
            default_value = web_request.get("default", None)
            
            cur = await self.get_value_from_config(section, key)
            row = dict(await cur.fetchone() or {})
            section_key = f"{section}.{key}"
            
            if not row and default_value is not None:
                item = {section_key: default_value}
                return {
                    "status": "success",
                    "result": item
                }
            elif row:
                item = {section_key: row['value']}
                return {
                    "status": "success",
                    "result": item
                }
                ...
                return {
                    "status": "error",
                    "message": f"Config item '{section_key}' not found"
                }
        except Exception as e:
            logging.exception(f"Error selecting config item: {e}")
            return {
                "status": "error",
                "message": f"Failed to select config item: {str(e)}"
            }
    
    async def _handle_config_select_all_request(self, web_request: WebRequest) -> Dict[str, Any]:
        return self.get_status()

    async def _handle_config_update_request(self, web_request: WebRequest) -> Dict[str, Any]:
        try:
            section = web_request.get_str("section")
            key = web_request.get_str("key")
            value = web_request.get("value")
            
            local_str = f"{section}.{key}"
            logging.info(f"local_str: {local_str}")
            if local_str != 'general.screen_state':
                await self.update_value_from_config(section, key, value)
            if local_str in self.config_items.keys():
                self.config_items[local_str] = value
                if local_str == "user.token":
                    self.server.send_event("account:user_login_status", len(self.config_items[local_str]) > 0)
            ...
            if local_str in target_keys:
                led_mode = self.config_items.get('general.chamber_light', 1)
                screen_state = self.config_items.get('general.screen_state', 1)
                
                kapis: KlippyAPI = self.server.lookup_component('klippy_apis')
                sync_cmd = f"SMART_STATUS MODE={led_mode} SCREEN={screen_state}"
                
                # 发送给 Klipper
                await kapis.run_gcode(sync_cmd)
                logging.info(f"LED Sync triggered by update: {sync_cmd}")
                
            section_key = f"{section}.{key}"
            item = {section_key: value}
            return {
                "status": "success",
                "result": item
            }
        except Exception as e:
            logging.exception(f"Error updating config item: {e}")
            return {
                "status": "error",
                "message": f"Failed to update config item: {str(e)}"
            }

    async def _handle_config_delete_request(self, web_request: WebRequest) -> Dict[str, Any]:
        try:
            section = web_request.get_str("section")
            key = web_request.get_str("key", None)
            
            await self.delete_value_from_config(section, key)
            if key:
                local_str = f"{section}.{key}"
                if local_str in self.config_items.keys():
                    self.config_items[local_str] = DEFAULT_CONFIGA_ITEM[local_str]
                    if local_str == "user.token":
                        self.server.send_event("account:user_login_status", len(self.config_items[local_str]) > 0)

                    if local_str == "user.region":
                        self.server.send_event("account:user_region", len(self.config_items[local_str]) > 0)

                return {
                    "status": "success",
                    "message": f"Config item '{section}.{key}' deleted successfully"
                }
            else:
                keys_to_reset = []
                for key_local, value in self.config_items.items():
                    if key_local.startswith(f"{section}."):
                        keys_to_reset.append(key_local)
                
                for reset_keys in keys_to_reset:
                    self.config_items[reset_keys] = DEFAULT_CONFIGA_ITEM[reset_keys]
                    if reset_keys == "user.token":
                        self.server.send_event("account:user_login_status", len(self.config_items[local_str]) > 0)

                    if reset_keys == "user.region":
                        self.server.send_event("account:user_region", len(self.config_items[local_str]) > 0)

                return {
                    "status": "success",
                    "message": f"Config item '{section}' deleted successfully"
                }

        except Exception as e:
            logging.exception(f"Error deleting config item: {e}")
            return {
                "status": "error",
                "message": f"Failed to delete config item: {str(e)}"
            }

    async def _handle_config_update_batch_request(self, web_request: WebRequest) -> Dict[str, Any]: 
        try:
            # 1. 解析原始数据
            items = web_request.get_json_list("items", default=[])
            ...
            for item in items:
                ...
                value = item.get("value") # value 允许为空或 None，取决于业务需求

                # 3. 剔除空 key 或空 section
                if not section or not key:
                    logging.warning(f"Skipping invalid config item: {item}")
                    continue

                # 构造数据库所需的元组列表
                valid_records.append((section, key, value))
                result_details.append({f"{section}.{key}": value})

            # 4. 如果有有效记录，执行批量插入
            if valid_records:
                await self.update_value_from_config_batch(valid_records)
                for s, k, v in valid_records:
                    local_str = f"{s}.{k}"
                    if local_str in self.config_items:
                        self.config_items[local_str] = v
    ...
    # systemStats表的接口
    async def _handler_get_system_stats_by_range(self, web_request: WebRequest) -> Dict[str, Any]:
        try:
            start = web_request.get_int('start', 0)
            end = web_request.get_int('end', 0)
            if end == 0:
                end = int(time.time())
            
            # 健壮性检查：防止 start > end 导致的空查询
            if start > end:
                return {"code": 400, "message": "Invalid time range: start > end", "data": {}}
            
            rows = await self.lookup_system_stats(start, end)

            response = {
                "cpu_all": {}, "cpu0": {}, "cpu1": {}, 
                "cpu2": {}, "cpu3": {}, "memory": {}, "error": {}
            }
            for row in rows:
                ts = str(row['timestamp'])  # 协议要求 timestamp 作为 key，通常转为字符串
                
                response["cpu_all"][ts] = row.get("cpu_all", 0.0)
                response["cpu0"][ts] = row.get("cpu0", 0.0)
                response["cpu1"][ts] = row.get("cpu1", 0.0)
                response["cpu2"][ts] = row.get("cpu2", 0.0)
                response["cpu3"][ts] = row.get("cpu3", 0.0)
                response["memory"][ts] = row.get("memory", 0.0)
                if row.get("error"):
                    response["error"][ts] = row["error"]
            ...
            logging.exception(f"System Monitor API: Failed to fetch stats")
            return {
                "code": 500,
...
class SqliteProvider(Thread):
    ...
    def setup_database(self) -> None:
        ...
        if CONFIG_TABLE not in self._tables:      #创建默认config表
            self._create_config_tables()
        else:
            config_proto = inspect.cleandoc(
                f"""
                {CONFIG_TABLE} (
            ...
            self._ensure_table_registered(CONFIG_TABLE, config_proto, 1) # 补救老客户
        if SYSTEM_STATS_TABLE not in self._tables:
            self._create_system_stats_table()
        else:
            self._check_system_stats_upgrade()
            pass # 预留升级位置
        # Find namespaces
        cur = self.sync_conn.execute(
    ...
    def _create_config_tables(self) -> None:
        ...
        with self.sync_conn:
            self.sync_conn.execute(f"CREATE TABLE {config_proto}")
        self._save_registered_table(CONFIG_TABLE, config_proto, 1)

    def _create_system_stats_table(self) -> None:
        if SYSTEM_STATS_TABLE in self._tables:
            return
        # 定义原型
        stats_proto = inspect.cleandoc(
            """
            system_stats (
                timestamp INTEGER PRIMARY KEY,
                cpu_all REAL,
                cpu0 REAL, cpu1 REAL, cpu2 REAL, cpu3 REAL,
                memory REAL,
                error TEXT
            )
            """
        ...
        # 执行创建
        with self.sync_conn:
            self.sync_conn.execute(f"CREATE TABLE {stats_proto}")
        
        self._save_registered_table(SYSTEM_STATS_TABLE, stats_proto, 1)
    
    def _ensure_table_registered(self, table_name: str, prototype: str, version: int) -> None:
        """专门为已经存在但没登记的老表补办手续"""
        _, current_ver = self._lookup_registered_table(table_name)
        if current_ver == 0:
            logging.info(f"Late registering {table_name} to version {version}")
            # 这里传入该表当前对应的 prototype 字符串即可
            self._save_registered_table(table_name, prototype, version)

    def _check_system_stats_upgrade(self) -> None:
        """预留升级位置：当 TARGET_VERSION 提高时在此编写 ALTER TABLE"""
        TARGET_VERSION = 1 
        _, current_ver = self._lookup_registered_table(SYSTEM_STATS_TABLE)
    ...
    def get_value_from_config(
        ...
        cur = conn.execute(            
            f"SELECT value FROM {CONFIG_TABLE} WHERE section = ? AND key = ?",
            (section, key, )
        )
        cur.arraysize = 100
        return SqliteCursorProxy(self, cur)
    
    def update_value_from_config(
        self,
        conn: sqlite3.Connection,
        section: str,
        key: str,
        ...
        with conn:
            conn.execute(            
                f"INSERT OR REPLACE INTO {CONFIG_TABLE} (section, key, value) VALUES (?, ?, ?)",
                (section, key, value)
            )
    
    def update_value_from_config_batch(
        self,
        conn: sqlite3.Connection,
        records: List[Tuple[str, str, Any]] # 格式: [('user', 'token', 'abc'), ('system', 'ip', '1.1.1.1')]
    ) -> None:
        def generate_params():
            for section, key, val in records:
            # 在这里可以统一处理编码逻辑，比如 json.dumps(val)
                yield (section, key, val)
        with conn:
            conn.executemany(
                f"INSERT INTO {CONFIG_TABLE} (section, key, value) VALUES(?, ?, ?) "
                "ON CONFLICT(section, key) DO UPDATE SET value=excluded.value",
                generate_params()
            )

    def delete_value_from_config(
        self,
        conn: sqlite3.Connection,
        section: str,
        key: str | None
        ...
        with conn:
            if key == None:
                conn.execute(            
                    f"DELETE FROM {CONFIG_TABLE} WHERE section = ?",
                    (section, )
                )
            else:
                conn.execute(            
                    f"DELETE FROM {CONFIG_TABLE} WHERE section = ? AND key = ?",
                    (section, key,)
                )

    def get_all_by_section_from_config(
        self,
        conn: sqlite3.Connection,
        section: str
    ) -> SqliteCursorProxy:
        cur = conn.execute(            
            f"SELECT key, value FROM {CONFIG_TABLE} WHERE section = ?",
            (section, )
        )
        cur.arraysize = 100
        return SqliteCursorProxy(self, cur)
    ...
    def insert_system_stats(
        ...
        # 使用 with conn 自动处理事务 (commit/rollback)
        with conn:
            conn.execute(
                f"INSERT INTO {SYSTEM_STATS_TABLE} "
                "(timestamp, cpu_all, cpu0, cpu1, cpu2, cpu3, memory, error) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    timestamp,
                    stats.get('cpu_all'),
    ...
    def lookup_system_stats(
        ...
        # 按照时间戳升序排列，方便前端绘图
        cur = conn.execute(
            f"SELECT * FROM {SYSTEM_STATS_TABLE} "
    ...
    def prune_system_stats(
        ...
        with conn:
            cur = conn.execute(
                f"DELETE FROM {SYSTEM_STATS_TABLE} WHERE timestamp < ?",
                (cutoff_time,)
            )
            # 返回被删除的行数，用于日志记录
            return cur.rowcount
