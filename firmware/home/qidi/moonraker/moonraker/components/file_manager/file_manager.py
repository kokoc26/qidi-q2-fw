...
class FileManager:
    def __init__(self, config: ConfigHelper) -> None:
        ...
        self.server.register_endpoint(
            "/server/files/delete_batch_file", RequestType.DELETE, self._handle_batch_file_delete
        )
        ...
        self.register_data_folder(".cache")
        self.register_data_folder(".temp")
    ...
    async def finalize_upload(self,
        ...
        async with self.sync_lock:
            try:
                ...
                elif upload_info['ext'] == '.3mf':
                    result = await self._finish_3mf_upload(upload_info)
                else:
    ...
    def _parse_upload_args(self,
        ...
        return {
            ...
            "plateindex": upload_args.get("plateindex", "1"),
        }
    ...
    async def _finish_gcode_upload(
        ...
        if upload_info['start_print']:
            ...
            if can_start:
                ...
                try:
                    await kapis.start_print(upload_info['filename'], user=user, plateindex=upload_info['plateindex'])
    ...
    async def _finish_3mf_upload(
        self, upload_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        finfo = await self._process_uploaded_file(upload_info)
        await self.gcode_metadata.parse_3mf_thumbnail(
            upload_info['filename'], finfo).wait()
        ...
        try:
            ...
            if upload_info['unzip_ufp']:
                ...
                finfo = self.get_path_info(upload_info['dest_path'],
                                           upload_info['root'])
                finfo['_3mf_path'] = dest_path
                # finfo['_3mf_path'] = tmp_path
            else:
                dest_path = upload_info['dest_path']
                if upload_info["is_link"]:
                    dest_path = os.path.realpath(dest_path)
    ...
    async def _handle_batch_file_delete(self,
                                  web_request: WebRequest
                                  ) -> Dict[str, Any]:
        file_path_data = web_request.get('path')
        
        # 1. 如果传的是列表（批量删除）
        if isinstance(file_path_data, list):
            results = []
            for path in file_path_data:
                res = await self.delete_file(path)
                results.append(res)
            return {"status": "batch_completed", "details": results}
        
        # 2. 如果传的是字符串（单个删除，保持原逻辑兼容）
        elif isinstance(file_path_data, str):
            return await self.delete_file(file_path_data)
            
        # 3. 容错处理
        else:
            raise web_request.error("Invalid path format. Expected string or list of strings.")
...
class BaseFileSystemObserver:
    ...
    def parse_gcode_metadata(self, file_path: str) -> asyncio.Event:
        ...
        if ext == ".ufp":
            ...
            path_info['_3mf_path'] = file_path
            return self.gcode_metadata.parse_3mf_thumbnail(rel_path, path_info)
    ...
    def on_item_copy(self, root: str, item_path: StrOrPath) -> Optional[Awaitable]:
        ...
        if item_path.is_file() and item_path.suffix in VALID_GCODE_EXTS:
            ...
            return ret.wait()
        # elif item_path.is_file() and item_path.suffix == '.3mf':
        #     ret = self.parse_gcode_metadata(str(item_path))
        #     return ret.wait()
