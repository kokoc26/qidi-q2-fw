...
class PrusaSlicer(BaseSlicer):
    ...
    def parse_preset_colours(self) -> Optional[List[str]]:
        colour_data = regex_find_string(r";\sfilament_colour\s=\s(%S)", self.footer_data)
        if colour_data:
            colours = [c.strip() for c in colour_data.split(";") if c.strip()]
            return colours
        return None
    def parse_used_extruders(self) -> Optional[List[str]]:
        extruder_str = regex_find_string(r";\sused_extruders\s=\s(%S)", self.footer_data)
        if extruder_str:
            try:

                extruder_list = extruder_str.split(";")
                extruders = []
                for e in extruder_list:
                    if e.strip():
                        extruders.append(int(e.strip()))
                return extruders
            except ValueError:
                extruders = []
                return extruders
...
def generate_thumb_path(input_filepath: str, root_path: str, plant_index: int) -> str:
    ...
    if not input_filepath.startswith(root_path):
        print(f"Error: Input file path is not under the root path: {root_path}")
        return ""
    ...
    thumb_path = os.path.join(
        ".thumbs",
        sub_dir,              # 例如: "USB" 或 ""
        base_name,            # 例如: "Thicker Filament holder.gcode"
        f"plate_{plant_index}.png"         # 固定的缩略图文件名
    )
...
def extract_3mf(_3mf_path: str, dest_path: str) -> None:
    ...
    if not os.path.isfile(_3mf_path):
        ...
        sys.exit(-1)
    # thumb_name = thumb_small_name = ""
    # thumb_name = os.path.splitext(
    #     os.path.basename(dest_path))[0] + "_1-168x168.png"
    # thumb_small_name = os.path.splitext(
    #     os.path.basename(dest_path))[0] + "_1-96x96.png"


    # dest_thumb_path = dest_thumb_small_path = ""
    # dest_thumb_path = os.path.join(dest_thumb_dir, thumb_name)
    # dest_thumb_small_path = os.path.join(dest_thumb_dir, thumb_small_name)

    # dest_thumb_dir = os.path.join(os.path.dirname(dest_path), ".thumbs")
    # if not os.path.exists(dest_thumb_dir):
    #     os.mkdir(dest_thumb_dir)

    # pattern = re.compile(r'^Metadata/plate_(\d+)\.png$')

    plate_num = 1
    try:
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            # tmp_thumb_path = ""
            # tmp_thumb_small_path = ""
            # tmp_thumb_all_path = ""
            tmp_xml_path = ""
            ...
            with zipfile.ZipFile(_3mf_path) as zf:
                # if _3MF_THUMB_PATH in zf.namelist():
                #     tmp_thumb_path = zf.extract(
                #         _3MF_THUMB_PATH, path=tmp_dir_name)
                # if _3MF_THUMB_SMALL_PATH in zf.namelist():
                #     tmp_thumb_small_path = zf.extract(
                #         _3MF_THUMB_SMALL_PATH, path=tmp_dir_name
                #     )
            ...
            #     width = height = 0
            #     with Image.open(tmp_thumb_path) as im:
            #         width = im.width
            #         height = im.height
            #         thumb_name = os.path.splitext(os.path.basename(dest_path))[0] + f"_1-{width}x{height}.png"
            #     dest_thumb_path = os.path.join(dest_thumb_dir, thumb_name)
            #     shutil.move(tmp_thumb_path, dest_thumb_path)

            # if os.path.exists(tmp_thumb_all_path):
            #     width = height = 0
            #     with Image.open(tmp_thumb_all_path) as im:
            #         width = im.width
            #         height = im.height
            #         thumb_name = os.path.splitext(os.path.basename(dest_path))[0] + f"_1-{width}x{height}.png"
            #     dest_thumb_path = os.path.join(dest_thumb_dir, thumb_name)
            #     shutil.move(tmp_thumb_all_path, dest_thumb_path)

            # if os.path.exists(tmp_thumb_small_path):
            #     width = height = 0
            #     with Image.open(tmp_thumb_small_path) as im:
            #         width = im.width
            #         height = im.height
            #         thumb_small_name = os.path.splitext(os.path.basename(dest_path))[0] + f"_1-{width}x{height}.png"
            #     dest_thumb_small_path = os.path.join(dest_thumb_dir, thumb_small_name)
            #     shutil.move(tmp_thumb_small_path, dest_thumb_small_path)
            ...
            if os.path.exists(tmp_xml_path):
                ...
                # layers = plate.find('layer_filament_lists')
                # if layers is not None:
                #     layer = layers.find('layer_filament_list')
                #     if layer is not None:
                #         layer_ranges = layer.get('layer_ranges', '')
                #         logging.info(layer_ranges)
                #         ranges = layer_ranges.strip().split()
                #         logging.info(ranges)
                #         if len(ranges) >= 2:
                #             metadata['object_height'] = int(ranges[1])
            if os.path.exists(tmp_project_settings_path):
                with open(tmp_project_settings_path, "r", encoding="utf-8") as file:
                    project_settings_data = file.read()
                project_settings_json = json.loads(project_settings_data)
                metadata['preset_colours'] = project_settings_json.get('filament_colour')
                metadata['layer_height'] = project_settings_json.get('layer_height')
                metadata['nozzle_temperature_initial_layer'] = project_settings_json.get('first_layer_extr_temp')
                metadata['filament_type'] = ";".join(project_settings_json.get('filament_type'))
                metadata['filament_name'] = ";".join(project_settings_json.get('filament_settings_id'))
                if metadata.get('object_height') is not None and metadata.get('layer_height') is not None:
                    metadata['object_height'] = float((metadata.get('object_height') + 1) * float(metadata.get('layer_height')))
    ...
    #         with Image.open(dest_thumb_path) as im:
    #             data.append({
    #                     'width': im.width, 'height': im.height,
    #                     'size': os.path.getsize(dest_thumb_path),
    #                     'relative_path': '.thumbs/' + thumb_name
    #             })
    #     if os.path.exists(dest_thumb_small_path):
    #         with Image.open(dest_thumb_small_path) as im:
    #             data.append({
    #                     'width': im.width, 'height': im.height,
    #                     'size': os.path.getsize(dest_thumb_small_path),
    #                     'relative_path': '.thumbs/' + thumb_small_name
    #             })
    ...
    #         'size': 0,
    #         'relative_path': generate_thumb_path(dest_path, "/home/qidi/printer_data/gcodes/", i+1)
    #     })
    data = []
    data.append({
        'width': 512, 'height': 512,
...
def main(path: str,
    ...
    if _3mf is not None:
        _3mf_data = extract_3mf(_3mf, file_path)
    metadata: Dict[str, Any] = {}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", "--_3mf", metavar="<3mf file>", default=None,
        help="optional path of 3mf file to process"
    )
    parser.add_argument(
    ...
    main(args.path, args.filename, args.ufp, args._3mf, check_objects)
