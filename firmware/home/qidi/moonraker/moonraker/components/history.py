...
class History:
    def __init__(self, config: ConfigHelper) -> None:
        ...
        self.server.register_endpoint(
            "/server/history/update/one_job_id", RequestType.POST,
            self._handle_update_one_job_id
        )
    ...
    async def _handle_update_one_job_id(
        self, web_request: WebRequest
    ):
        job_id_hex = web_request.get_str("job_id")
        update_fields = web_request.get_str("update_fields")
        update_value = web_request.get("update_value")
        # 参数验证
        if not job_id_hex:
            return {"error": "job_id is empty!"}
        
        if not update_fields:
            return {"error": "update's fields is empty!"}
        
        if update_value is None:
            return {"error": "update's value is empty!"}
        
        # 检查字段名
        ALLOWED_FIELDS = {
            'user', 'filename', 'status', 'start_time', 
            'end_time', 'print_duration', 'total_duration',
            'filament_used', 'instance_id'
        }
        if update_fields not in ALLOWED_FIELDS:
            return {"error": f"field '{update_fields}' is not allowed!"}
        # 执行更新
        job_id = int(job_id_hex, 16)
        async with self.history_table as tx:
            await tx.execute(
                f"UPDATE {HIST_TABLE} SET {update_fields} = '{update_value}' "
    ...
    async def _on_job_state_changed(
        ...
        if job_event == JobEvent.STARTED:
            ...
            script = f'SDCARD_RESET_FILE'
            await kapis.run_gcode(script)
            ...
            script = f'SDCARD_RESET_FILE'
            await kapis.run_gcode(script)
        elif job_event == JobEvent.STANDBY:
            await self.finish_job("cancelled", prev_stats)
    ...
    async def _publish_job_state_mqtt(
        self,
        job_event: str,
        stats: Dict[str, Any],
    ) -> None:
        """发布任务状态变化到MQTT"""
        try:
            self.mqtt_client = self.server.lookup_component('mqtt')
        except Exception:
            self.mqtt_client = None
        ...
        try:
            ...
            mqtt_payload = {
                "job_state": job_event,
                "uid": stats.get("uid", "native"),
                "timestamp": stats.get("print_duration", 0),
                "filename": stats.get("filename", ""),
                "plateindex": stats.get("plateindex", "1"),
                "user": getattr(self, 'job_user', 'No User')
            }
            
            # 发布到MQTT主题
            topic = f"{self.mqtt_client.instance_name}/job_state/changed"
            await self.mqtt_client.publish_topic(topic, mqtt_payload)
            
            logging.debug(f"Published job state change to MQTT: {mqtt_payload}")
            
        except Exception as e:
            logging.error(f"Failed to publish job state to MQTT: {e}")
