...
class ProcStats:
    ...
    async def _handle_stat_update(self, eventtime: float) -> float:
        ...
        # 3. 遍历映射并动态添加状态
        for component_name, attr_name in STATUS_MAP.items():
            try:
                component = self.server.lookup_component(component_name)
                if component_name == 'mqtt':
                    event_data[attr_name] = component.get_status(eventtime)['status']
                else:
                    event_data[attr_name] = component.get_status(eventtime)
            except Exception as e:
                event_data[attr_name] = None 
        self.server.send_event("proc_stats:proc_stat_update", event_data)
        if (
        ...
        if not self.update_sequence % 60:
            try:
                cpu_data = self.cpu_usage
                mem_data = self.memory_usage
                self.server.lookup_component("database").log_system_stats(cpu_data, mem_data)
            except Exception:
                logging.exception("System Monitor: Failed to push periodic stats to database")
