...
CORE_COMPONENTS = [
    ...
    'http_client', 'announcements', 'webcam', 'extensions', 'moonraker_status_update'
]
...
class Server:
    ...
    def load_components(self) -> None:
        ...
        cfg_sections.discard('mqtt')
        cfg_sections.add('mqtt')
        # load remaining optional components
        cfg_sections.discard('frp_manager')
        cfg_sections.add('frp_manager')
    ...
    def lookup_components(
        ...
        """
        通用的组件查找函数
        :param filter_func: 接收 (name, component) 并返回 bool 的回调函数
        """
        try:
            # 浅拷贝字典，确保遍历时的线程/协程安全
            items = dict(self.components).items()
    ...
    def remove_failed_component(self, component_name: str) -> None:
        if component_name in self.failed_components:
...
def main(from_package: bool = True) -> None:
    ...
    try:
        ...
        logging.info(f"Linux system has run {uptime_seconds}s")
    except Exception as e:
        logging.error(f"Failed to read system uptime: {e}")
